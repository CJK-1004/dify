...
class PBTSchema:
    """
    PBT API 요청에 필요한 스키마를 정의하는 데이터 클래스

    Attributes:
        pbt_type (str): API 요청 타입 ('workflow', 'agent', 'chat', 'completion')
    ...
    def get_schema(self) -> Dict:
        ...
        if self.pbt_type == "workflow":
            return {
                "inputs": {},
                "response_mode": self.response_mode,
                "user": self.user,
            }
        elif self.pbt_type == "agent":
            return {
                "inputs": {},
            ...
            raise ValueError(
                "Invalid pbt_type. Must be 'completion', 'workflow', 'agent', or 'chat'"
            )
