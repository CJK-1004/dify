const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'PBTアカウント',
    langGeniusAccountTip: 'PBTアカウントと関連するユーザーデータ。',
    editName: '名前を編集',
  ...
  members: {
    ...
    invitationSentTip: '招待が送信され、彼らはPBTにサインインしてあなた様のチームデータにアクセスできます。',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'PBTが提供するOpenAIホスティングサービスを使用すると、GPT-3.5などのモデルを使用できます。トライアルクォータが使い果たされる前に、他のモデルプロバイダを設定する必要があります。',
      callTimes: '通話回数',
  ...
  apiBasedExtension: {
    title: 'API拡張機能は、PBTのアプリケーション全体での簡単な使用のための設定を簡素化し、集中的なAPI管理を提供します。',
  ...
  about: {
    changeLog: '変更ログ',
    updateNow: '今すぐ更新',
    nowAvailable: 'PBT {{version}} が利用可能です。',
    latestAvailable: 'PBT {{version}} が最新バージョンです。',
  },
  appMenus: {
    overview: '監視',
    promptEng: 'オーケストレート',
